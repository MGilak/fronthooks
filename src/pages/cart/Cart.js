...
const Cart = () => {
  ...
  const finalPriceEn = (price, discount) => {
    const finalPrice = price - price * (discount / 100);
    return finalPrice;
  };

  const toFasi = (price) => {
    const coursePrice = replace(toFarsiNumber(price));
    return coursePrice;
  };

  // قیمت اولیه
  const priceFa = (price) => {
    return replace(toFarsiNumber(price));
  };
  ...
  const totalPay = () => {
    const totalPrice = cart.reduce(
      (acc, course) => acc + finalPriceEn(course.price, course.discount),
      0
    );
    ...
    <div>
      ...
      <main>
        <div className="container mx-auto mt-4 lg:max-w-screen-lg">
          {cart.length > 0 ? (
            <div class="flex items-center text-gray-800 my-6">
              <h1 class="font-extrabold text-xl">سبد خرید شما</h1>
            </div>
          ) : (
            <div class="flex flex-col text-gray-600 my-6">
              <h1 class="font-extrabold text-lg">
                دوره‌ای در سبد خرید شما وجود ندارد
              </h1>

              <Link to="/courses">
                <h1 className="text-blue-500 font-bold text-xl mt-4">
